...
- **Pinned CPU relay buffer**(`d011b4b`):老 `copy_worker_func`
  每个 worker 线程直接分配 **1 GB fp16 = 2 GB pinned 内存**。在
  driver-580 / `RLIMIT_MEMLOCK=1GB` 的 V100 上,这 2 GB 注册失败
  会以误导性的 "CUDA out of memory" 报出,服务器连 DHT socket 都
  bind 不上就崩了。新默认值 **4 MB**,可通过
  `BLOOMBEE_PIN_RELAY_MB` 覆盖。正是这个修改让 V100 上两台 llama-7b
  服务器能共存 —— **mainline 在 16 GB V100 上压根跑不起两台
  llama-7b**,原因就是这个 pinned buffer。
...
|---|---:|---:|---:|
| mainline | **启动失败**(pinned-buffer OOM) | — | — |
| arch-reform PAGED_KV=0 | 81.2 | 80.8 | ~210 |
| arch-reform PAGED_KV=1 | 82.8 | 82.9 | ~205 |

双服务器场景 **只有 arch-reform 能跑**,靠的就是 `d011b4b`。
mainline 在第二台服务器启动时直接撞 1 GB pinned buffer OOM。

## 6. Pinned-buffer 副作用(回答 4 MB 的问题)

变更:`copy_worker_func` 的默认 pinned CPU relay buffer
**1 GB → 4 MB**;可通过 `BLOOMBEE_PIN_RELAY_MB`(整数 MB)覆盖。

**这个 buffer 什么时候真的用**:仅当拷贝的一端在 CUDA 上
(disk-offload 中转、压缩权重解压中转)。`Disk% = 0`(V100 上 llama-7b
跑默认就是这样)时 **根本不会有拷贝入队** —— buffer 纯浪费。

**缩小默认值的副作用**:

1. **正确性**:没有副作用。`size > cpu_buf.numel()` 的守卫会 fallback
   到直接 `dst_data.copy_(src_data)`,不会静默 OOB view。
2. **大 transfer 的性能**:fallback 路径是阻塞拷贝,丢掉 pinned-DMA
   overlap。大模型激进 offload 场景(llama-65b、bloom-176b、mixtral
   aggressive offload)里,单次 transfer > 4 MB 时会有吞吐损失。
   估算:seq_len=512 × head_dim=128 × num_heads=32 × fp16 的单个
   KV 块 ≈ 8 MB,模型稍微大点就会超过这个门槛。
3. **Mitigation**:`BLOOMBEE_PIN_RELAY_MB=1024` 恢复原行为。
   **大 GPU 主机**(A100 / H100、`RLIMIT_MEMLOCK` unlimited 或 pinned
   预算充足)做真正的 offload 时推荐设这个。
4. **为什么不是 0**:小块 relay transfer 还是能从 pinned DMA overlap
   受益。4 MB 的选取标准是"小模型下够装一个 KV slab page 或一个
   transformer 权重分片,同时不触发 V100 注册失败"。

**给租到大 GPU 之后的下一步工作**:直接从
`BLOOMBEE_PIN_RELAY_MB=1024` 起(或按主机 pinned 预算再调高),
这样你拿到的是 reform 之前的吞吐天花板;4 MB 默认值只是 V100
可行性旋钮,不是性能决策。
...
- [ ] 设 `BLOOMBEE_PIN_RELAY_MB=1024`(或按 host 预算再高)。
- [ ] 确认 `RLIMIT_MEMLOCK=unlimited`(`ulimit -l`);否则 pinned
      buffer 还是会撞一样的墙。
